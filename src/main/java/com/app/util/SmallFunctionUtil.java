 ...
 */
public class SmallFunctionUtil {
    private SmallFunctionUtil(){}
    
    public static Logger logger = Logger.getLogger("Yu_SmallFunctionUtil");
    
    public static void main(String[] args) {
		System.out.println(getSysJournalNo(20,false));
	}
    
    /**
     * 获取系统流水号
     * @see 若欲指定返回值的长度or是否全由数字组成等,you can call {@link SmallFunctionUtil#getSysJournalNo(int, boolean)}
     * @return 长度为20的全数字
     */
...
/*    *//**
    ...
    *//**
    ...
    public static String getStringFromIoBuffer(IoBuffer buffer, int size, String charset){
        ...
        try {
        	...
        	logger.error("字符解码异常,自动切换第二种解码方式,本次的堆栈信息如下", e);
            try {
                result = new String(buffer.array(), charset);
            } catch (UnsupportedEncodingException ee) {
                logger.error("字符解码异常,系统不支持该字符集[" + charset + "],本次的堆栈信息如下", ee);
            }
        }
        return result;
    }
    */
    ...
    //TODO  完成一个姓名的中间效果
    public static String getNameSimple(String data){
     ...
     * @see And the calls {@link SmallFunctionUtil#getHexSign(String,String,String,boolean)}进行签名
     ...
     * @see It will Calls {@link SmallFunctionUtil#getBytes(String str, String charset)}
